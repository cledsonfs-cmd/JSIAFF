...
/**
 * This class provides Iterator for the image files (jpg and png) in the specified directory
 * next() method  loads and returns BufferedImage objects
 * @author Jon Tait
 */
public class ImagesIterator implements Iterator<BufferedImage> {
    ...
    public ImagesIterator(File dir) throws IOException {
        ...
        AmostraController ac = new AmostraController();
       ...
       /* try {
            imageFiles = ac.getListFileImagens();
        } catch (Exception ex) {
            Logger.getLogger(ImagesIterator.class.getName()).log(Level.SEVERE, null, ex);
        }*/
    ...
    public ImagesIterator(List<Amostra> valores) throws IOException {
        List<File> imageFiles = new ArrayList<File>();
        for (Amostra a : valores) {
            File file = new File("");
            //contruir o file com os dados do BD
            imageFiles.add(file);
        }
