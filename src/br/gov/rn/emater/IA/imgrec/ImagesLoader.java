...
//			System.out.println(filenameOfCurrentImage + " is: " + img.getWidth() + "x" + img.getHeight());
    ...
    public static Map<String, FractionRgbData> getFractionRgbDataForDb(List<Amostra> amostras, Dimension samplingResolution,int x) throws IOException {
        if (amostras.size()==0) {
            throw new IOException("numero de amostras invalido: " + amostras.size());
        }

        Map<String, FractionRgbData> rgbDataMap = new HashMap<String, FractionRgbData>();
        AmostraController amostraController = new AmostraController();
        for (Amostra amostra : amostras) {
            BufferedImage img = amostraController.ImageIconBufferedImage(amostra.getImagem(x));
            img = ImageSampler.downSampleImage(samplingResolution, img);
            String filenameOfCurrentImage = amostra.getIdamostra() + "_" + amostra.getDoenca().getNomecientifico().trim();
//			System.out.println(filenameOfCurrentImage + " is: " + img.getWidth() + "x" + img.getHeight());
            //StringTokenizer st = new StringTokenizer(filenameOfCurrentImage, ".");
            //rgbDataMap.put(st.nextToken(), new FractionRgbData(img));
